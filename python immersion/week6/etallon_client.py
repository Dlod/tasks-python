...
def run(host, port):
    ...
    try:
        client1.sock_connection.sendall(b"malformed command test\n")
        client1._read_answer()
        client2.sock_connection.sendall(b"malformed command test\n")
        client2._read_answer()
    except ClientError:
        pass
    else:
